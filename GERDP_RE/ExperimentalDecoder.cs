    ...
    internal class ExperimentalDecoder : IDecoder
    {
        ...
        public void Decode(ResDataSet set, FileInfo res_file) //这个level不想改了，就这么摆烂
        {
            ...
            for (int i = 0; i < set.count; i++)
            {
                ...
                {
                    Warning($"[合法错误][{res_relative_path}][{set.name}][{br.BaseStream.Position.ToString("X")}]{i + 1}/{set.count}.{location.hex_origin}是0，跳过这个对象！");
                    continue;
                }
                ...
                Task.Run(() =>
                {
                    ...
                    {
                        Warning($"[小型提示][{set.name}][{br.BaseStream.Position.ToString("X")}]{i + 1}/{set.count}.是已经存在的文件:{file_name}，没有必要再次覆盖这个文件。");
                        return;
                    }
                    ...
                    {
                        Info($"[信息][{set.name}][{br.BaseStream.Position.ToString("X")}]{i + 1}/{set.count}.是存在的BLZ4文件，优化为复制任务。");
                    ...
                    //}
                        ...
                        lock (FileWriterLocker)
                        {
                            ...
                            if (File.Exists(file_name))
                            {
                                Warning($"[小型提示][{set.name}][{br.BaseStream.Position.ToString("X")}]{i + 1}/{set.count}.因为旧的文件是存在的所以没有存储{file_name}，新文件大小:{save_data.LongLength}，如果你看到了这个，说明多线程有一些问题，但这个问题被兜底了。");
                            }
                            else
                            {
                            ...
                            if (!BLZ4MD5Map.ContainsKey(blz4_md5))
                            {
                                Info($"[信息][{set.name}][{br.BaseStream.Position.ToString("X")}]{i + 1}/{set.count}.添加这个文件到BLZ4的解压记录中。");
                        ...
                        var md5 = CryptUtils.GetMD5HashFromBytes(save_data);

                        if (!ResMD5HashSet.Contains(md5) && IsRES(save_data))
                        {
                            ResMD5HashSet.Add(md5);
